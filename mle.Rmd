...
$$ 
\sigma(\mu) = c_\sigma \mu^{1+b_\sigma}(1-\mu)^{1+a_\sigma}
$$

```{r}
F_sigma = function(mu, aa=3.11, bb=2.14, cc=1.74){
...
counts_MLE = function(alpha, counts, q=6, bvm=13){
  ...
  par_O$pSig$cc = 1
  pinit = c(tildecounts=10.5, Sa=1/30, aa=0.1, bb=0.1, cc=0.1, sz=0.1)
  ...
  c(F_b(out[1], 13), F_Sa(out[2]), abs(out[3]), abs(out[4]), abs(out[5]), abs(out[6]))
}
```
...
```{r Fit it}
counts_MLE(alpha_list, counts_list) -> fits
fits
```